...
class GeminiConfig:
    ...
    def _load_env_config(self) -> Dict[str, Any]:
        ...
        if os.getenv("GEMINI_FLASH_MODEL"):
            env_config.setdefault("models", {}).setdefault("nicknames", {})["flash"] = os.getenv("GEMINI_FLASH_MODEL")
        
        if os.getenv("GEMINI_PRO_MODEL"): 
            env_config.setdefault("models", {}).setdefault("nicknames", {})["pro"] = os.getenv("GEMINI_PRO_MODEL")
        
        # Timeout overrides
        if os.getenv("CLI_TIMEOUT"):
            try:
                env_config.setdefault("timeouts", {})["cli_timeout"] = int(os.getenv("CLI_TIMEOUT"))
            except ValueError:
                logger.warning(f"Invalid CLI_TIMEOUT value: {os.getenv('CLI_TIMEOUT')}")
        
        if os.getenv("API_TIMEOUT"):
            try:
                env_config.setdefault("timeouts", {})["api_timeout"] = int(os.getenv("API_TIMEOUT"))
            except ValueError:
        ...
        if os.getenv("MAX_FILE_SIZE"):
            try:
                env_config.setdefault("limits", {})["max_file_size"] = int(os.getenv("MAX_FILE_SIZE"))
            except ValueError:
                logger.warning(f"Invalid MAX_FILE_SIZE value: {os.getenv('MAX_FILE_SIZE')}")
        
        if os.getenv("MAX_LINES"):
            try:
                env_config.setdefault("limits", {})["max_lines"] = int(os.getenv("MAX_LINES"))
            except ValueError:
                logger.warning(f"Invalid MAX_LINES value: {os.getenv('MAX_LINES')}")
        
        if os.getenv("MAX_PROMPT_SIZE"):
            try:
                env_config.setdefault("limits", {})["max_prompt_size"] = int(os.getenv("MAX_PROMPT_SIZE"))
            except ValueError:
                logger.warning(f"Invalid MAX_PROMPT_SIZE value: {os.getenv('MAX_PROMPT_SIZE')}")
        
        if os.getenv("MAX_CODEBASE_SIZE"):
            try:
                env_config.setdefault("limits", {})["max_codebase_size"] = int(os.getenv("MAX_CODEBASE_SIZE"))
            except ValueError:
                logger.warning(f"Invalid MAX_CODEBASE_SIZE value: {os.getenv('MAX_CODEBASE_SIZE')}")
        
        if os.getenv("MAX_CONTEXT_WINDOW"):
            try:
                env_config.setdefault("limits", {})["max_context_window"] = int(os.getenv("MAX_CONTEXT_WINDOW"))
            except ValueError:
                logger.warning(f"Invalid MAX_CONTEXT_WINDOW value: {os.getenv('MAX_CONTEXT_WINDOW')}")
        
        if os.getenv("SANITIZATION_MAX_LENGTH"):
            try:
                env_config.setdefault("limits", {})["sanitization_max_length"] = int(os.getenv("SANITIZATION_MAX_LENGTH"))
            except ValueError:
                logger.warning(f"Invalid SANITIZATION_MAX_LENGTH value: {os.getenv('SANITIZATION_MAX_LENGTH')}")
        ...
        if os.getenv("ENABLE_MARKDOWN_CONVERSION"):
            ...
            if value in ("true", "1", "yes", "on", "false", "0", "no", "off"):
                env_config.setdefault("execution", {})["enable_markdown_conversion"] = parsed_value
        ...
        if os.getenv("ENABLE_SANITIZATION"):
            ...
            # Only set if it's a recognized boolean value
            if value in ("true", "1", "yes", "on", "false", "0", "no", "off"):
                env_config.setdefault("security", {})["enable_sanitization"] = parsed_value
    ...
    def _validate_and_normalize(self) -> None:
        ...
        for timeout_key, timeout_value in timeouts.items():
            if not isinstance(timeout_value, (int, float)) or timeout_value <= 0:
                logger.warning(f"Invalid timeout value for {timeout_key}: {timeout_value}, using default")
        ...
        for limit_key, limit_value in limits.items():
            if not isinstance(limit_value, int) or limit_value <= 0:
                logger.warning(f"Invalid limit value for {limit_key}: {limit_value}, using default")
        ...
        for nickname, model_name in nicknames.items():
            if not isinstance(model_name, str) or not model_name.strip():
                logger.warning(f"Invalid model name for nickname {nickname}: {model_name}")
                nicknames[nickname] = self.DEFAULT_CONFIG["models"]["nicknames"].get(nickname, "gemini-2.5-flash")
        ...
        for tool_name, assigned_nickname in assignments.items():
            if assigned_nickname not in valid_nicknames:
                logger.warning(f"Invalid model assignment for {tool_name}: {assigned_nickname}, using 'flash'")
    ...
    def _apply_force_model_override(self) -> None:
        ...
        if force_model:
            ...
            if force_model in nicknames:
                ...
                assignments = self._config.setdefault("models", {}).setdefault("assignments", {})
                ...
                nicknames = self._config.setdefault("models", {}).setdefault("nicknames", {})
    ...
    def get_model(self, tool_name: str, explicit_model: Optional[str] = None) -> str:
        ...
        if explicit_model is not None:
            if isinstance(explicit_model, str) and explicit_model.strip():
                ...
                logger.warning(f"Invalid explicit model for {tool_name}: {explicit_model}, falling back to default")
        ...
        if not model_name or not isinstance(model_name, str):
            logger.warning(f"Invalid model name for {tool_name}, falling back to default")
...
def register_tool_if_missing(tool_name: str, default_model_nickname: str) -> None:
    ...
    if tool_name not in assignments:
        ...
        if default_model_nickname not in nicknames:
            logger.warning(f"Unknown model nickname '{default_model_nickname}' for tool '{tool_name}', using 'flash'")
            default_model_nickname = "flash"
        
        assignments[tool_name] = default_model_nickname
        logger.info(f"Registered new tool '{tool_name}' with default model '{default_model_nickname}'")
    else:
        logger.debug(f"Tool '{tool_name}' already registered with model '{assignments[tool_name]}'")
