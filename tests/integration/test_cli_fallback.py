        ...
        for malicious_prompt in malicious_prompts:
            ...
            with patch('asyncio.create_subprocess_exec', return_value=mock_process) as mock_exec:
                with patch('gemini_mcp_server.GOOGLE_API_KEY', None):
                    ...
                    result = await execute_gemini_cli_streaming(malicious_prompt, "gemini_quick_query")
                    ...
                    assert call_args == ("gemini", "-m", "gemini-2.5-flash", "-p", malicious_prompt)
    ...
    async def test_command_argument_validation(self):
        ...
        for dangerous_model in dangerous_models:
            with patch('gemini_mcp_server.GEMINI_MODELS', {"flash": dangerous_model}):
                result = await execute_gemini_cli_streaming("test", "gemini_quick_query")
...
class TestCLIProcessManagement:
    ...
    async def test_stderr_error_capture(self):
        ...
        stderr_with_sensitive = b"Error: API key AIzaSyBHJ5X2K9L8M3N4O5P6Q7R8S9T0 invalid"
    ...
    async def test_concurrent_cli_executions(self):
        ...
        for i in range(3):
            ...
            mock_process.stdout.readline = AsyncMock(return_value=b"")
            mock_process.communicate = AsyncMock(return_value=(f"Output {i}".encode(), b""))
...
class TestCLIInputValidation:
    ...
    async def test_prompt_length_validation(self):
        ...
        result = await execute_gemini_cli_streaming(very_long_prompt, "gemini_quick_query")
    ...
    async def test_prompt_type_validation(self):
        ...
        for invalid_prompt in invalid_prompts:
            result = await execute_gemini_cli_streaming(invalid_prompt, "gemini_quick_query")
    ...
    async def test_empty_prompt_handling(self):
        ...
        for empty_prompt in empty_prompts:
            result = await execute_gemini_cli_streaming(empty_prompt, "gemini_quick_query")
