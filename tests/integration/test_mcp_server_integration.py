...
class TestMCPServerIntegration:
    ...
    async def test_tool_security_validation(self):
        ...
        for dangerous_query in dangerous_queries:
            with patch('gemini_mcp_server.execute_gemini_cli_streaming') as mock_exec:
                ...
                result = await call_tool("gemini_quick_query", {"query": dangerous_query})
    ...
    async def test_large_input_handling(self):
        ...
        result = await call_tool("gemini_analyze_code", {"code_content": many_lines_code})
        assert len(result) == 1
...
"""
        ...
        with patch('gemini_mcp_server.execute_gemini_cli_streaming') as mock_exec:
            mock_exec.return_value = {"success": True, "output": "Code analysis complete"}
    ...
    async def test_codebase_analysis_path_security(self):
        ...
        with patch('gemini_mcp_server.validate_path_security') as mock_validate:
            with patch('gemini_mcp_server.execute_gemini_cli_streaming') as mock_exec:
                ...
                mock_exec.return_value = {"success": True, "output": "Analysis complete"}
