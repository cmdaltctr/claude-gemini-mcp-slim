...
class TestExecuteGeminiCliStreamingLimits:
    ...
    async def test_execute_gemini_cli_streaming_under_limit_test_mode(self):
        ...
        assert "A" * 50 in result["output"]  # First 50 chars of prompt should be in dummy response
    ...
    async def test_execute_gemini_cli_streaming_over_limit_blocked(self):
        ...
        result = await execute_gemini_cli_streaming(
            ...
            task_type="gemini_quick_query",
            _test_mode=True  # Even in test mode, validation should block
        )
    ...
    async def test_full_flow_with_prompt_sanitization_under_limit(self):
        ...
        with patch('claude_gemini_mcp.gemini_mcp_server.execute_gemini_cli_streaming') as mock_streaming:
            ...
            result = await call_tool("gemini_quick_query", {"query": query, "context": context})
    ...
    async def test_full_flow_with_oversized_context_sanitization(self):
        ...
        oversized_context = "A" * 100000  # Much larger than the sanitization limit for context
        
        # Mock execute_gemini_cli_streaming to capture calls
        with patch('claude_gemini_mcp.gemini_mcp_server.execute_gemini_cli_streaming') as mock_streaming:
            mock_streaming.return_value = {
                "success": True, 
                "output": "Python response"
            }
            
            # Call the tool
            result = await call_tool("gemini_quick_query", {"query": query, "context": oversized_context})
            ...
            assert len(call_args) < 10000  # Much smaller than original oversized context
    ...
    async def test_prompt_injection_sanitization_with_valid_execution(self):
        ...
        with patch('claude_gemini_mcp.gemini_mcp_server.execute_gemini_cli_streaming') as mock_streaming:
            ...
            # Call the tool
            result = await call_tool("gemini_quick_query", {"query": dangerous_query, "context": context})
            ...
            assert "[filtered-content]" in call_args  # Dangerous content should be replaced
    ...
    async def test_integration_with_config_limits(self):
        ...
        with patch('claude_gemini_mcp.gemini_mcp_server.cfg.get_limit') as mock_get_limit:
    ...
    async def test_mock_assert_not_called_for_oversized_prompt(self):
        ...
        with patch('claude_gemini_mcp.gemini_mcp_server.execute_gemini_cli_streaming') as mock_streaming:
            ...
            try:
                ...
                result = await call_tool("gemini_analyze_code", {"code_content": oversized_code})
    ...
    async def test_mock_assert_called_once_for_valid_prompt(self):
        ...
        # Mock the execute_gemini_cli_streaming function
        with patch('claude_gemini_mcp.gemini_mcp_server.execute_gemini_cli_streaming') as mock_streaming:
            mock_streaming.return_value = {"success": True, "output": "Analysis complete"}
            
            # Call the tool with valid content
            result = await call_tool("gemini_analyze_code", {"code_content": valid_code})
