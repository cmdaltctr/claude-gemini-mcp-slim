...
class TestJSONConfigDiscovery(unittest.TestCase):
    ...
    def test_extract_mcp_api_key_success(self):
        ...
        config = {
            "mcpServers": {
                "gemini-mcp": {"env": {"GOOGLE_API_KEY": "test-api-key-1234567890"}}
            }
        }
    ...
    def test_get_from_json_config_invalid_key(self):
        ...
        config_data = {
            "mcpServers": {
                "gemini-mcp": {"env": {"GOOGLE_API_KEY": "short"}}  # Too short
            }
        }
...
class TestSharedMCPPathDetection(unittest.TestCase):
    ...
    def test_add_shared_mcp_path_env_info_success(self, mock_home, mock_sys_path):
        ...
        def mock_path_exists(self):
            path_str = str(self)
            return (
                "env-info.json" in path_str
                or "site-packages" in path_str
                or "google/generativeai" in path_str
            )
        ...
        with patch.object(Path, "exists", mock_path_exists):
            with patch("builtins.open", mock_open(read_data=json.dumps(env_info_data))):
                ...
                mock_sys_path.insert.assert_called_once_with(
                    0, "/path/to/site-packages"
                )

    @patch("sys.path")
    @patch("pathlib.Path.home")
    @patch("platform.python_version")
    def test_add_shared_mcp_path_fallback_detection(
        self, mock_python_version, mock_home, mock_sys_path
    ):
        ...
        def mock_path_exists(self):
            path_str = str(self)
            return (
                "python3.12/site-packages" in path_str
                or "google/generativeai" in path_str
            )
    ...
    def test_add_shared_mcp_path_json_error(self, mock_home):
        ...
        def mock_path_exists(self):
            ...
            return (
                "env-info.json" in path_str
            )  # Only env-info.json exists, no fallback paths
    ...
    def test_add_shared_mcp_path_missing_generativeai(self, mock_home, mock_sys_path):
        ...
        def mock_path_exists(self):
            ...
            return "env-info.json" in path_str or (
