...
def test_load_precedence_explicit_args():
    ...
    assert (
        config.get_limit("max_file_size", -100)
        == config.DEFAULT_CONFIG["limits"]["max_file_size"]
    )
    assert (
        config.get_timeout("api_timeout", 0)
        == config.DEFAULT_CONFIG["timeouts"]["api_timeout"]
    )
...
def test_invalid_environment_variables(monkeypatch):
    ...
    assert (
        config.get_timeout("cli_timeout")
        == config.DEFAULT_CONFIG["timeouts"]["cli_timeout"]
    )
    assert (
        config.get_limit("max_file_size")
        == config.DEFAULT_CONFIG["limits"]["max_file_size"]
    )
    assert (
        config.get_security_setting("enable_sanitization")
        == config.DEFAULT_CONFIG["security"]["enable_sanitization"]
    )
