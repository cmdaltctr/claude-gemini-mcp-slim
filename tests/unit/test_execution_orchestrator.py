...
class TestProcessResultOutput:
    ...
    def test_process_successful_result_no_conversion(self):
        ...
        processed = _process_result_output(
            result, convert_markdown=False, show_progress=False
        )
    ...
    def test_process_failed_result(self):
        ...
        processed = _process_result_output(
            result, convert_markdown=True, show_progress=False
        )
    ...
    @patch(
        "claude_gemini_mcp.helpers.execution_orchestrator.MARKDOWN_UTILS_AVAILABLE",
        True,
    )
...
class TestExecuteGeminiSmart:
    ...
    @patch("claude_gemini_mcp.helpers.execution_orchestrator.execute_gemini_api")
    @patch(
        "claude_gemini_mcp.helpers.execution_orchestrator.execute_gemini_cli_streaming"
    )
    @patch("claude_gemini_mcp.helpers.execution_orchestrator.cfg")
    @pytest.mark.asyncio
    async def test_api_failure_cli_fallback(
        self, mock_cfg, mock_cli_exec, mock_api_exec, mock_get_api_key
    ):
    ...
    @patch("claude_gemini_mcp.helpers.execution_orchestrator.cfg")
    @pytest.mark.asyncio
    async def test_no_api_key_direct_cli(
        self, mock_cfg, mock_cli_exec, mock_get_api_key
    ):
    ...
    @pytest.mark.asyncio
    async def test_model_selection_fallback(
        self, mock_cli_exec, mock_get_api_key, mock_cfg
    ):
        ...
        result = await execute_gemini_smart(
            "Test prompt", task_type="analyze_code", show_progress=False
        )
...
class TestExecuteGeminiSmartWithProgress:
    ...
    def test_progress_success_with_streaming(
        self, mock_split, mock_smart_exec, mock_create_progress
    ):
        """Test successful execution with progress indicators and streaming."""
        # Setup mocks
        mock_progress = MagicMock()
        mock_create_progress.return_value = mock_progress
        mock_smart_exec.return_value = {
            "success": True,
