...
@pytest.mark.skipif(not USE_REAL_API or not TEST_API_KEY, 
...
"""
        ...
        with patch('gemini_mcp_server.GOOGLE_API_KEY', TEST_API_KEY):
            result = await call_tool("gemini_analyze_code", {
            ...
            assert any(keyword in response_text for keyword in [
                "recursion", "performance", "optimization", "exponential", "memoization"
            ])
...
"""
            ...
            assert "path traversal" in response.lower() or "validation" in response.lower()
...
"""
                ...
                result = await call_tool("gemini_codebase_analysis", {
                    "directory_path": "./test_analysis_dir",
                    "analysis_scope": "all"
                })
    ...
    async def test_error_handling_workflow(self):
        ...
        with patch('gemini_mcp_server.execute_gemini_api') as mock_api:
            with patch('gemini_mcp_server.execute_gemini_cli_streaming') as mock_cli:
                ...
                # Mock API failure
                mock_api.return_value = {"success": False, "error": "API quota exceeded"}
                
                # Mock successful CLI fallback
                mock_cli.return_value = {"success": True, "output": "CLI fallback response"}
...
"""
    ...
    async def test_performance_benchmarks(self):
        ...
        with patch('gemini_mcp_server.execute_gemini_cli_streaming') as mock_exec:
            ...
            result = await call_tool("gemini_quick_query", {"query": "Performance test"})
    ...
    async def test_concurrent_requests_workflow(self):
        ...
        with patch('gemini_mcp_server.execute_gemini_cli_streaming') as mock_exec:
            ...
            def mock_response(prompt, task_type):
                return {"success": True, "output": f"Response for {task_type}: {prompt[:20]}..."}
