...
MAX_LINES = 800  # Maximum lines for code analysis
CLI_TIMEOUT = 60  # 60 seconds timeout for CLI operations


def get_api_key() -> Optional[str]:
    """Get API key using simplified discovery approach"""
    # Option 1: Environment variable (for docker/env file usage)
    ...
    try:
        claude_code_config = Path.home() / ".config" / "claude-code" / "mcp_config.json"
    ...
    # Option 3: Claude Desktop MCP configuration
    try:
    ...
    # Option 4: Project-level .env file in gemini folder
    try:
...
def validate_file_security(file_path: str) -> tuple[bool, str, Optional[Path]]:
    ...
    try:
        ...
        if file_stat.st_size > MAX_FILE_SIZE * 2:  # Allow some buffer
            return (
                False,
                f"File too large: {file_stat.st_size} bytes (max: {MAX_FILE_SIZE * 2})",
                None,
            )
...
def execute_gemini_cli(
    ...
    try:
        ...
        if show_progress:
            ...
            print(f"⏱️ Timeout: {CLI_TIMEOUT} seconds", file=sys.stderr)
        ...
        while True:
            ...
            if elapsed > CLI_TIMEOUT:
                if show_progress:
                    print(
                        f"\n⚠️ Timeout reached after {CLI_TIMEOUT}s, terminating...",
                        file=sys.stderr,
                    )
            ...
            if show_progress and time.time() - last_progress_time > 15:
                ...
                remaining_seconds = max(0, CLI_TIMEOUT - elapsed_seconds)
                print(
        ...
        if show_progress:
            print("-" * 50, file=sys.stderr)
            if timeout_reached:
                print(f"⏰ Operation timed out after {CLI_TIMEOUT}s", file=sys.stderr)
            else:
        ...
        if timeout_reached:
            return {
                "success": False,
                "error": f"CLI timeout after {CLI_TIMEOUT} seconds",
        ...
        return {"success": False, "error": f"CLI timeout after {CLI_TIMEOUT} seconds"}
