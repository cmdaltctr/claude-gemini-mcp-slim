...
def demo_basic_usage():
    """Demo basic usage of the progress utility"""
    print("=== Basic Usage Demo ===")

    # Dots progress
    progress = create_dots_progress("API Request")
    progress.start()
    time.sleep(2)

    # Simulate streaming response
    chunks = ["Hello", " World", "!", " This", " is", " streaming", " content."]
    for chunk in chunks:
...
# Demo different progress indicator types
def demo_different_types():
    """Demo different progress indicator types"""
    print("\n=== Different Progress Types Demo ===")

    types = [
        ("Dots", create_dots_progress("Processing")),
        ("Spinner", create_spinner_progress("Analyzing")),
        ("Pulse", create_pulse_progress("Loading")),
        ("Bar", create_bar_progress("Computing")),
    ]

    for name, progress in types:
        print(f"\n{name} Progress:")
        progress.start()
        time.sleep(2)
        progress.stream_chunk(f"{name} completed successfully!")
        progress.complete()
        time.sleep(0.5)
...
def demo_context_manager():
    ...
    with create_spinner_progress("Task").progress_context() as progress:
        time.sleep(1.5)
        progress.stream_chunk("Context manager automatically handles cleanup!")
        time.sleep(0.5)
...
def demo_error_handling():
    ...
    progress = create_pulse_progress("Operation")
    progress.start()
    time.sleep(1)
    progress.stop("Operation failed due to network error")
