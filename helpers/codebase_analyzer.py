...
def discover_files(
    root_path: Union[str, Path], config: Optional[Dict[str, Any]] = None
) -> List[FileInfo]:
    """
    ...
    """
    discoverer = FileDiscoverer(Path(root_path), config or {})
    return discoverer.discover_files()


def aggregate_contents(
    files: List[FileInfo], max_total_size: int = 200_000
) -> Dict[str, Any]:
    """
...
def build_project_report(
    ...
    """
    analyzer = ProjectStructureAnalyzer(Path(root_path), {})
    structure = analyzer.build_project_report(files, aggregated_content)

    detector = TechStackDetector(Path(root_path), {})
