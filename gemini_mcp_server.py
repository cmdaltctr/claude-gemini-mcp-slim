...
async def execute_gemini_cli_streaming(
    ...
    try:
        ...
        while True:
            try:
                ...
                if line:
                    ...
                    logger.info(
                        f"Gemini output: {decoded_line.strip()[:100]}{'...' if len(decoded_line.strip()) > 100 else ''}"
                    )
        ...
        if remaining_stdout:
            ...
            logger.info(
                f"Final output: {decoded_remaining.strip()[:100]}{'...' if len(decoded_remaining.strip()) > 100 else ''}"
            )
