...
def run_integration_tests_parallel() -> int:
    ...
    return run_command([
        sys.executable, "-m", "pytest",
        "tests/integration/",
        "-n", "2",              # Limited parallel execution
        "--timeout=30",         # 30 second timeout per test
        "--timeout-method=thread",  # Thread-based timeout
        "--no-cov",            # Skip coverage for speed
        "-v",                  # Verbose output
...
def main():
    ...
    parser.add_argument(
        "mode",
        choices=[
            "install", "fast", "integration", "integration-parallel", 
            "all", "smoke", "debug", "specific"
        ],
        help="Test execution mode"
    )
    parser.add_argument(
        "--test-pattern",
        help="Specific test pattern to run (for specific/debug modes)"
    )
    parser.add_argument(
        "--timeout",
        type=int,
